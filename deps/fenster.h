...
struct fenster {
  const char *title;
  int width;  /* updated on WM_SIZE; buf may need realloc */
  int height;
  uint32_t *buf;
  ...
  int x;
  int y;
  int mouse;      /* bit 0 = left, bit 1 = right, bit 2 = middle */
  int wheel;      /* accumulated mouse wheel delta (notches) */
  int pending_w;  /* WM_SIZE stashes new size here; consumer applies + reallocs */
  int pending_h;
#if defined(__APPLE__)
  id wnd;
#elif defined(_WIN32)
...
static LRESULT CALLBACK fenster_wndproc(HWND hwnd, UINT msg, WPARAM wParam,
  ...
  case WM_MOUSEMOVE:
    ...
    break;
  case WM_MOUSEWHEEL:
    f->wheel += (int)(short)HIWORD(wParam) / WHEEL_DELTA;
    break;
  case WM_SIZE: {
    int nw = LOWORD(lParam), nh = HIWORD(lParam);
    if (nw > 0 && nh > 0) { f->pending_w = nw; f->pending_h = nh; }
  } break;
  case WM_KEYDOWN:
  case WM_KEYUP: {
