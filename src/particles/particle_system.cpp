...
void ParticleSystem::emit(ParticleType type, int count, const Object& src,
                          ...
                          int cf_rand_override) {
    if (type >= ParticleType::COUNT || count <= 0) return;
    const TypeData& t = TYPES[static_cast<int>(type)];
    // &46e4 trick: callers can rewrite cf_base for one emit. cf_rand
    // is normally the type's stock mask (preserves icer green/yellow
    // pairing etc); the override is a port-only debug knob — see header.
    uint8_t cf_base = (cf_base_override < 0)
                     ? t.cf_base
                     : static_cast<uint8_t>(cf_base_override);
    uint8_t cf_rand = (cf_rand_override < 0)
                     ? t.cf_rand
                     : static_cast<uint8_t>(cf_rand_override);
    ...
    for (int k = 0; k < count; k++) {
        ...
        // &220d-&2215: colour and flags. cf_base/cf_rand are type
        // defaults OR per-call overrides (see top of emit()).
        uint8_t cf = (rng.next() & cf_rand) ^ cf_base;
